...
{   	 	
	...
	public function edit_user($data)
	{
		...
		if($data['acu_password'] == $data['acu_password_hide'])	
		{
			$data['acu_password'] = $data['acu_password'];
		}
		else
		{
			$data['acu_password'] = md5($data['acu_password']);
		}
		unset($data['acu_password_hide']);
	...
	public function myprofile($id,$role)
	{
		if($role == 'Administrator')
		{
			...
			$res['name'] = $res['au_name'];			
			$res['email'] = $res['au_email'];
			$res['password'] = $res['au_password'];			
			return $res;
		}
		else if($role == 'Accountant')
		{
			...
			$res['id'] = $res['acu_id'];	
			$res['role'] = 'Accountant';
			$res['name'] = $res['acu_name'];			
			$res['email'] = $res['acu_email'];
			$res['password'] = $res['acu_password'];				
			return $res;
		}
		else if($role == 'Dispatcher')
		{
			...
			$res['id'] = $res['du_id'];	
			$res['role'] = 'Dispatcher';
			$res['name'] = $res['du_name'];			
			$res['email'] = $res['du_email'];
			$res['password'] = $res['du_password'];					
			return $res;
		}
		else
		{
	...
	public function edit_profile($data)
	{
		...
		{
			$data_update['au_name'] = $data['name'];

			$data_update['au_password'] =  ($data['old_password'] != $data['password']) ? md5($data['password']) : $data['old_password'];

			$this->db->where('au_id', $data['id']);
			$this->db->update('tbl_admin_user', $data_update);
			return $this->db->affected_rows() == 1 ? '1' : '0';
		}
		else if($data['role'] == 'Accountant')
		{
			$data_update['acu_name'] = $data['name'];
			$data_update['acu_password'] =  ($data['old_password'] != $data['password']) ? md5($data['password']) : $data['old_password'];

			$this->db->where('acu_id', $data['id']);
			$this->db->update('tbl_accounting_user', $data_update);
			return $this->db->affected_rows() == 1 ? '1' : '0';
		}
		else if($data['role'] == 'Dispatcher')
		{
			$data_update['du_name'] = $data['name'];
			$data_update['du_password'] =  ($data['old_password'] != $data['password']) ? md5($data['password']) : $data['old_password'];

			$this->db->where('du_id', $data['id']);
			$this->db->update('tbl_dispatch_user', $data_update);
			return $this->db->affected_rows() == 1 ? '1' : '0';
		}
	}
}
