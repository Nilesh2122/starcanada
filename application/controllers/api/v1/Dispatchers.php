...
class Dispatchers extends REST_Controller {
    ...
    function __construct()
    {
		...
		$this->load->model('v1/ModelDispatchers');    
    }	
    ...
    {	
		...
		if(!$data['du_email'] || !$data['du_password'] || !$data['device_type'] || !$data['device_token'])
		{
			$this->_sendResponse(4);
		}
		else
		{		
			$result = $this->ModelDispatchers->login_process($data);
	...
	public function badge_count_post()
	{
		...
		{	
			$result = $this->ModelDispatchers->badge_count($data);
