...
class ModelDriver extends CI_Model {
	...
	public function update_op_status($data, $driver)
	{
		...
		//Accepted ::$operation[0]['op_ref_id']
		$ndata['title'] = 'Operation Accepted!';
		$ndata['body'] = 'Operation : '.$operation[0]['op_c_load_reference'].' accepted by driver, Click to view details.';
		...
		if($data['op_status'] == '11')
		{
			...
			$ndata['body'] = 'Driver has rejected Operation : '.$operation[0]['op_c_load_reference'];
		...
		if($data['op_status'] == '5')
		{
			...
			$ndata['body'] = 'Loading has been successful for Operation : '.$operation[0]['op_c_load_reference'].' and its on the way to delivery, Click to track.';
			$ndata['redirect'] = base_url().'index.php/operations/track/'.$operation[0]['op_id'];
		}
		...
		if($data['op_status'] == '7')
		{
			...
			$ndata['body'] = 'Operation : '.$operation[0]['op_c_load_reference'].' has been delivered successfuly, Click to track.';
			$ndata['redirect'] = base_url().'index.php/operations/track/'.$operation[0]['op_id'];
		}				
		...
		if($this->db->affected_rows() == 1)
		{
			...
			return array('success'=>true, 'status_code'=>1, 'message'=>'Success!', 'data'=>$return[0]);
		}
		else
		{
